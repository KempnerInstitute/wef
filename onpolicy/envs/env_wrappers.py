...
def worker(remote, parent_remote, env_fn_wrapper):
    ...
    while True:
        ...
        if cmd == 'step':
            ...
            remote.send(None)  # Acknowledge the command
        elif cmd == 'render':
            mode, kwargs = data
            if mode == "rgb_array":
                fr = env.render(mode=mode, **kwargs)
                remote.send(fr)
            elif mode == "human":
                env.render(mode=mode, **kwargs)
        elif cmd == 'save_svg_snapshot':
            filepath = env.save_svg_snapshot(data['filepath'], env_index=data['env_index'])
            remote.send(filepath)
        elif cmd == 'reset_task':
            ob = env.reset_task()
            remote.send(ob)
        elif cmd == 'close':
...
class GuardSubprocVecEnv(ShareVecEnv):
    ...
    def set_attr(self, attr_name, attr_value):
        """
        Set an attribute in all environments.
        """
        for remote in self.remotes:
            remote.send(('set_attr', (attr_name, attr_value)))
        for remote in self.remotes:
...
class SubprocVecEnv(ShareVecEnv):
    def __init__(self, env_fns, spaces=None):
        ...
        self.waiting = False
        self.closed = False
        self.envs = [fn() for fn in env_fns]  # TODO revisit if causing slowdown
        nenvs = len(env_fns)
        self.remotes, self.work_remotes = zip(*[Pipe() for _ in range(nenvs)])
        self.ps = [Process(target=worker, args=(work_remote, remote, CloudpickleWrapper(env_fn)))
                   for (work_remote, remote, env_fn) in zip(self.work_remotes, self.remotes, env_fns)]
    ...
    def reset(self):
        ...
        # print("obs", obs)
        # print("obs.shape for each:", [ (o[0].shape, o[1].shape) for o in obs])
        return np.stack(obs)

    def reset_with_seeds(self, seeds):
        for remote, seed in zip(self.remotes, seeds):
            remote.send(('reset_with_seed', seed))
            # note that env_seed may not be updated on env object
            # due to lack of explicit remote attribute setting
        obs = [remote.recv() for remote in self.remotes]
        return np.stack(obs)
    ...
    def render(self, mode="rgb_array", num_envs=None, **kwargs):
        # make separate kwargs for each env
        # split up attentions if it is in kwargs,
        # but keep other kwargs as is
        attentions = kwargs.pop('attentions', None)
        remotes = self.remotes[:num_envs] if num_envs is not None else self.remotes
        for i, remote in enumerate(remotes):
            env_kwargs = dict(kwargs)
            if attentions is not None:
                env_kwargs['attentions'] = attentions[i]
            remote.send(('render', (mode, env_kwargs)))
        if mode == "rgb_array":
            frames = [remote.recv() for remote in remotes]
            return np.stack(frames)


    def save_svg_snapshot(self, filepath):
        for i, remote in enumerate(self.remotes):
            kwargs = {'filepath': filepath, 'env_index': i}
            remote.send(('save_svg_snapshot', kwargs))
        svgs = [remote.recv() for remote in self.remotes]
        return svgs
...
def shareworker(remote, parent_remote, env_fn_wrapper):
    ...
    while True:
        ...
        if cmd == 'step':
            ...
            filename = env.save_svg_snapshot(data['filename'], env_index=data['env_index'])
            remote.send(filename)
        elif cmd == 'close':
            env.close()
            remote.close()
            break
...
class DummyVecEnv(ShareVecEnv):
    ...
    def reset_with_seeds(self, seeds):
        obs = [env.reset(seed) for (env, seed) in zip(self.envs, seeds)]
        return np.array(obs)
    ...
    def render(self, mode="human", num_envs=None, **kwargs):
        envs = self.envs[:num_envs] if num_envs is not None else self.envs
        if mode == "rgb_array":
            return np.array([env.render(mode=mode, **kwargs) for env in envs])
        elif mode == "human":
            for env in envs:
                env.render(mode=mode, **kwargs)
        else:
    ...
    def save_svg_snapshot(self, filename):
        return np.array([env.save_svg_snapshot(filename, i) for i, env in enumerate(self.envs)])
